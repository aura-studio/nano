...
func (c *CodecEntity) EncodeMessage(m *message.Message) ([]byte, error) {
	...
	// encode flag
	flag := byte(m.Type)
	code, err := message.Route(m.VersionNum, m.Route)
	...
	// encode version ID
	binary.BigEndian.PutUint32(buf[offset:], m.VersionNum)
	offset += 4
...
func (c *CodecEntity) DecodeMessage(data []byte) (*message.Message, error) {
	...
	// decode version ID
	m.VersionNum = binary.BigEndian.Uint32(data[offset:])
	offset += 4
	...
	if c.compressed.Load() {
		...
		route, err := message.Code(m.VersionNum, uint32(code))
		if err != nil {
			return nil, err
		}
		m.Route = route
